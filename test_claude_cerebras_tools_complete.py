#!/usr/bin/env python3
"""
Complete Tool Execution Test Suite for claude-cerebras
...
def run_claude_cerebras_test(prompt, test_name, timeout=60):
    """Run a claude-cerebras command and capture all output"""
    log_test(f"Starting test: {test_name}")
    
    # Create temp log file for this test
    log_file = f"/tmp/claude_cerebras_test_{test_name.replace(' ', '_')}.log"
    
    try:
        # Run claude-cerebras with output redirection
        cmd = f"./claude-cerebras -p '{prompt}' > {log_file} 2>&1"
...
def analyze_logs_for_tool_execution(log_file):
    ...
    tool_indicators = [
        "CEREBRAS TOOL EXECUTION STARTED",
        "Executing bash:",
        "File operation:",
        "CEREBRAS TOOL EXECUTION COMPLETED"
    ]
...
def check_cerebras_api_key():
    """Check if Cerebras API key is available"""
    api_key = os.getenv('CEREBRAS_API_KEY')
    if not api_key:
        log_test("❌ CEREBRAS_API_KEY not set - tests may fail")
        return False
    else:
        log_test("✅ CEREBRAS_API_KEY is set")
        return True

def main():
    """Run comprehensive claude-cerebras tool execution tests"""
    log_test("=== Claude-Cerebras Tool Execution Test Suite ===")
    
    # Check prerequisites
    if not check_cerebras_api_key():
        log_test("⚠️  Warning: CEREBRAS_API_KEY not set. Tests may not work properly.")
    
    # Clean up from previous tests
    test_files = [
        "test_cerebras_file_create.txt",
        "test_cerebras_file_edit.txt", 
        "test_cerebras_bash_output.txt"
    ]
    ...
    tests = [
        {
            'name': 'file_creation',
            'prompt': 'Create a file called test_cerebras_file_create.txt with the content "Hello from claude-cerebras tools"',
            'verify_file': 'test_cerebras_file_create.txt',
            'expected_content': 'Hello from claude-cerebras tools'
        },
        {
            'name': 'bash_execution',
            'prompt': 'Run the command "echo \'Cerebras bash test\' > test_cerebras_bash_output.txt"',
            'verify_file': 'test_cerebras_bash_output.txt',
            'expected_content': 'Cerebras bash test'
        },
        {
            'name': 'file_editing',
            'prompt': 'Create a file test_cerebras_file_edit.txt with "original content", then edit it to say "cerebras edited content"',
            'verify_file': 'test_cerebras_file_edit.txt',
            'expected_content': 'cerebras edited content'
        }
    ]
    ...
    for test in tests:
        ...
        # Run the test
        result = run_claude_cerebras_test(test['prompt'], test['name'])
    ...
    results_file = "/tmp/claude_cerebras_test_results.json"
